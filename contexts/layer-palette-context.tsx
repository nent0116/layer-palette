...
function createDefaultNodes(template: LayerMap["template"]): LayerNode[] {
        ...
        children: [
          {
            ...
            children: [
              ...
              {
                title: "SNS投稿「年末年始の振り返り」",
                notes: "担当者名: SNS担当者\n開始日: 2024-01-04\n終了日: 2024-01-05\nステータス: 完了\n進捗率: 100%"
              },
              {
                title: "ニュースレター配信",
                notes: "担当者名: マーケティング担当者\n開始日: 2024-01-06\n終了日: 2024-01-07\nステータス: 完了\n進捗率: 100%"
              }
            ]
          },
          {
  ...
  if (template === "wbs" || template === "content-calendar") {
    // WBSテンプレートとコンテンツカレンダーテンプレートの場合は階層構造を作成
    const createHierarchicalNodes = (nodes: any[], level = 0): LayerNode[] => {
      return nodes.map((node, index) => ({
        id: generateId(),
        title: node.title,
        notes: node.notes,
        backgroundColor: getColorForLevel(template, level),
        children: node.children ? createHierarchicalNodes(node.children, level + 1) : [],
        order: index,
        level,
      }))
    }
    return createHierarchicalNodes(templates[template])
  }
